...
export default function FinanceiroPage() {
  const { buildings, loading, organizationId, bankBalance, monthlyExpenses, monthlyProfit } = usePortfolio();
  ...
  return <div className="content"><div className="page-heading"><div><div className="eyebrow"><CircleDollarSign size={13} /> Fluxo financeiro</div><h1>Financeiro</h1><p className="subtitle">Receitas exclusivamente dos aluguéis informados nos imóveis.</p></div><div className="page-heading-actions"><Link href="/alugueis" className="button button-primary"><ArrowUpRight size={14} /> Registrar aluguéis</Link><Link href="/despesas" className="button button-ghost"><Receipt size={14} /> Gerenciar despesas</Link></div></div>
    ...
    <div className="panel section-gap"><div className="panel-heading"><div><h2>Histórico financeiro</h2><p>Confirmações de aluguel entram como crédito; quando desfeitas, ficam registradas como saída.</p></div></div>{history.length ? <div className="table-wrap"><table><thead><tr><th>Evento</th><th>Descrição</th><th>Data</th><th>Valor</th></tr></thead><tbody>{history.map((entry) => <tr key={entry.id}><td><span className={`status ${entry.event_type === "credit" ? "status-alugado" : "status-vago"}`}>{entry.event_type === "credit" ? "Crédito" : "Saída"}</span></td><td>{entry.description}</td><td>{new Date(entry.occurred_at).toLocaleString("pt-BR")}</td><td className={entry.event_type === "credit" ? "positive" : "negative"}><strong>{entry.event_type === "credit" ? "+" : "−"} {brl(Number(entry.amount || 0))}</strong></td></tr>)}</tbody></table></div> : <div className="empty-state" style={{ minHeight: 120 }}><p>Nenhum evento financeiro automático registrado.</p></div>}</div>
  </div>;
}
