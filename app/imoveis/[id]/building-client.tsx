...
export default function BuildingDetailClient() {
  ...
  useEffect(() => {
    ...
    async function loadPrimaryPhotos() {
      ...
      if (!supabase) return;
      const result = await supabase.from("documents").select("id, unit_id, storage_path").eq("organization_id", organizationId).eq("category", "photo").eq("is_primary", true).in("unit_id", unitIds);
      if (result.error) return;
      const entries = await Promise.all((result.data ?? []).map(async (row) => ({ unitId: String(row.unit_id), url: (await supabase.storage.from("organization-documents").createSignedUrl(String(row.storage_path), 3600)).data?.signedUrl })));
    ...
    return () => { active = false; };
  }, [building?.unitsData, organizationId]);
  ...
  async function openFiles(unit: PropertyUnit) {
    ...
    setFileLoading(true);
    setMessage("");
    const result = await supabase.from("documents").select("id, name, category, storage_path, mime_type, is_primary").eq("organization_id", organizationId).eq("unit_id", unit.id).order("created_at", { ascending: false });
    if (result.error) setMessage(result.error.message);
    else {
  ...
  async function loadPrimaryPhotos() {
    const unitIds = (building?.unitsData ?? []).map((unit) => unit.id);
    const supabase = createSupabaseBrowserClient();
    if (!supabase || !unitIds.length) return;
    const result = await supabase.from("documents").select("unit_id, storage_path").eq("organization_id", organizationId).eq("category", "photo").eq("is_primary", true).in("unit_id", unitIds);
    if (result.error) return;
    const entries = await Promise.all((result.data ?? []).map(async (row) => ({ unitId: String(row.unit_id), url: (await supabase.storage.from("organization-documents").createSignedUrl(String(row.storage_path), 3600)).data?.signedUrl })));
  ...
  async function uploadDocuments(event: React.ChangeEvent<HTMLInputElement>, category: "photo" | "contract") {
    ...
    if (!files.length || !filesUnit) return;
    const supabase = createSupabaseBrowserClient();
    if (!supabase) return;
    setFileLoading(true);
    setMessage("");
  ...
  return <div className="content">
    ...
    <div className="panel section-gap"><div className="panel-heading"><div><h2>Unidades e contratos</h2><p>{units.length} resultados · fotos, arquivos e edição rápida disponíveis.</p></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><input className="table-filter" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar unidade, tipo ou inquilino" /><select className="filter-select" value={filter} onChange={(event) => setFilter(event.target.value)}><option value="todos">Todos</option><option value="alugados">Alugados</option><option value="vagos">Vagos</option><option value="contratos">Contratos vencendo</option><option value="reajustes">Reajustes próximos</option><option value="venda">À venda</option></select><button className="button button-ghost button-small" onClick={() => setCreatingUnit(true)} disabled={role === "viewer"}><Plus size={13} /> Nova unidade</button></div></div>
      ...
      <div className="table-wrap"><table><thead><tr><th>Foto</th><th>Unidade</th><th>Tipo</th><th>Inquilino</th><th>Status</th><th>Aluguel mensal</th><th>Vencimento</th><th>Último pagamento</th><th>Início</th><th>Fim</th><th>Próximo reajuste</th><th>Ações</th></tr></thead><tbody>{units.map((unit) => { const isRented = unit.status === "alugado" || unit.status === "venda_alugado" || unit.rent > 0; const statusLabel = unit.status === "venda_alugado" || (unit.status === "venda" && isRented) ? labels.venda_alugado : labels[unit.status]; return <tr key={unit.id}><td><div className="unit-photo-thumb">{primaryPhotos[unit.id] ? <img src={primaryPhotos[unit.id]} alt={`Foto principal de ${unit.code}`} /> : <ImageIcon size={18} />}</div></td><td><strong>{unit.code}</strong><small><Ruler size={11} /> Qtd. {unit.quantity ?? 1}</small></td><td>{unit.type}</td><td>{unit.tenantName ?? "Não cadastrado"}</td><td><span className={`status status-${unit.status === "venda" || unit.status === "venda_alugado" ? "vago" : unit.status}`}>{statusLabel}</span></td><td><strong>{unit.rent ? brl(unit.rent) : "—"}</strong></td><td>{unit.lease?.dueDay ? `Dia ${unit.lease.dueDay}` : "—"}</td><td>{unit.lease?.lastPaymentDate ? new Date(`${unit.lease.lastPaymentDate.slice(0, 10)}T12:00:00`).toLocaleDateString("pt-BR") : "—"}</td><td>{unit.lease?.startDate ?? "—"}</td><td>{unit.lease?.endDate ?? "—"}</td><td>{unit.lease?.nextAdjustmentDate ?? "—"}</td><td><button className="icon-btn" aria-label={`Fotos e arquivos de ${unit.code}`} onClick={() => void openFiles(unit)}><Paperclip size={14} /></button><button className="icon-btn" aria-label={`Editar ${unit.code}`} onClick={() => setEditing(unit)}><Edit3 size={14} /></button></td></tr>; })}</tbody></table></div>
    </div>
