...
export default function BuildingDetailClient() {
  ...
  const [buildingForm, setBuildingForm] = useState({ name: "", value: "", city: "", state: "", address: "", lastValuationDate: "", status: "active", notes: "", saleProximity: false, attention: false, attentionNote: "" });
  ...
  async function deleteUnit() {
    if (!editing || !organizationId || role === "viewer") return;
    if (!window.confirm(`Excluir definitivamente a unidade “${editing.code}”? Todos os contratos, pagamentos, documentos, despesas e registros vinculados serão excluídos.`)) return;
    const supabase = createSupabaseBrowserClient();
    if (!supabase) return;
    setFileLoading(true);
    const result = await supabase.rpc("delete_property_unit", { target_org: organizationId, target_unit: editing.id });
    if (result.error) setMessage(result.error.message === "not_authorized" ? "Seu perfil não pode excluir unidades." : "Não foi possível excluir a unidade.");
    else { setEditing(null); setMessage("Unidade e todos os dados vinculados foram excluídos."); await refresh(); }
    setFileLoading(false);
  }
