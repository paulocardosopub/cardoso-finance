...
export default function ImoveisPage() {
  ...
  const visible = useMemo(() => buildings.filter((building) => {
    ...
    const matchesFilter = filter === "todos" || (filter === "venda" ? building.status === "venda" : filter === "ocupados" ? building.occupied > 0 : building.occupied < building.units);
  ...
  return <div className="content"><div className="page-heading"><div><div className="eyebrow"><Building2 size={13} /> Portfólio imobiliário</div><h1>Imóveis</h1><p className="subtitle">Unidades, contratos e valores sincronizados com o Supabase.</p></div><Link href="/onboarding" className="button button-primary"><Plus size={15} /><span>Novo imóvel</span></Link></div>
    ...
    <div className="panel"><div className="panel-heading"><div><h2>Todos os prédios e grupos</h2><p>{visible.length} agrupamentos · dados do banco</p></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><label className="search-inline"><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar" /></label><select className="filter-select" value={filter} onChange={(event) => setFilter(event.target.value)}><option value="todos">Todos</option><option value="ocupados">Com ocupação</option><option value="vagos">Com vagas</option><option value="venda">À venda</option></select><SlidersHorizontal size={14} style={{ alignSelf: "center", color: "#8490a5" }} /></div></div><div className="building-list">{visible.map((building) => <Link href={`/imoveis/${building.id}`} key={building.id} className="building-row" style={{ padding: 16 }}><div className="building-thumb" style={{ width: 62, height: 62 }} /><div className="building-info"><strong style={{ fontSize: 14 }}>{building.name}</strong><small><MapPin size={11} style={{ verticalAlign: "-2px" }} /> {building.city}, {building.state} · {building.units} unidades</small><div style={{ display: "flex", gap: 8, marginTop: 10 }}><span className="tag">{building.status}</span><span className="tag">{building.occupied} ocupadas</span></div></div><div className="building-value"><strong style={{ fontSize: 15 }}>{compactBrl(building.value)}</strong><small>{brl(building.revenue)} / mês</small><div style={{ marginTop: 9, width: 100 }}><div className="progress"><span style={{ width: `${building.units ? (building.occupied / building.units) * 100 : 0}%` }} /></div></div></div></Link>)}</div></div>
  </div>;
}
