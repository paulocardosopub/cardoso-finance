...
export default function ImoveisPage() {
  const { buildings, loading, organizationId, role, memberVisibility, memberSummary, expenses, refresh } = usePortfolio();
  ...
  const visible = useMemo(() => buildings.filter((building) => {
    ...
    const matchesFilter = filter === "todos" || filter === "vendidos" || (filter === "venda" ? isForSale(building) : filter === "proximos" ? Boolean(building.saleProximity) : filter === "atencao" ? Boolean(building.attention) : filter === "ocupados" ? building.occupied > 0 : building.occupied < building.units);
