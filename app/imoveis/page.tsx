...
export default function ImoveisPage() {
  ...
  if (role === "employee") return <EmployeeProperties buildings={buildings} buildingPhotos={buildingPhotos} query={query} setQuery={setQuery} filter={filter} setFilter={setFilter} />;
...
function EmployeeProperties({ buildings, buildingPhotos, query, setQuery, filter, setFilter }: { buildings: Building[]; buildingPhotos: Record<string, string>; query: string; setQuery: (value: string) => void; filter: string; setFilter: (value: string) => void }) {
  const visible = useMemo(() => buildings.filter((building) => {
    if (filter === "vendidos") return building.status === "vendido";
    if (building.status === "vendido") return false;
    const text = `${building.name} ${building.city} ${building.state}`.toLowerCase();
    ...
    return matches && (filter === "todos" || (filter === "ocupados" && occupied) || (filter === "vagos" && !occupied) || (filter === "venda" && forSale));
  }), [buildings, filter, query]);
  return <div className="content"><div className="page-heading"><div><div className="eyebrow"><Building2 size={13} /> Operação imobiliária</div><h1>Imóveis</h1><p className="subtitle">Consulte e atualize ocupação, aluguel, inquilinos, fotos e contratos.</p></div></div><div className="metrics"><div className="metric-card"><div className="metric-top"><span>Imóveis</span><span className="metric-icon"><Building2 size={15} /></span></div><div className="metric-value">{buildings.length}</div><div className="metric-foot">Carteira operacional</div></div><div className="metric-card"><div className="metric-top"><span>Ocupados</span><span className="metric-icon">✓</span></div><div className="metric-value">{buildings.reduce((sum, building) => sum + (building.unitsData ?? []).filter((unit) => unit.status === "alugado" || unit.status === "venda_alugado" || unit.lease).length, 0)}</div><div className="metric-foot">Unidades com inquilino</div></div><div className="metric-card"><div className="metric-top"><span>À venda</span><span className="metric-icon">$</span></div><div className="metric-value">{buildings.filter(isForSale).length}</div><div className="metric-foot">Imóveis ou unidades</div></div></div><div className="panel"><div className="panel-heading"><div><h2>Carteira operacional</h2><p>{visible.length} imóveis encontrados</p></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><label className="search-inline"><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar imóvel" /></label><select className="filter-select" value={filter} onChange={(event) => setFilter(event.target.value)}><option value="todos">Todos</option><option value="ocupados">Ocupados</option><option value="vagos">Vagos</option><option value="venda">À venda</option></select></div></div><div className="building-list">{visible.map((building) => <Link href={buildingPath(building)} key={building.id} className={`building-row ${isForSale(building) ? "sale-row" : ""}`}><div className="building-thumb" style={{ width: 62, height: 62 }}>{buildingPhotos[building.id] ? <img src={buildingPhotos[building.id]} alt={`Foto de ${building.name}`} /> : <Building2 size={18} />}</div><div className="building-info"><strong>{building.name}</strong><small>{[building.city, building.state].filter(Boolean).join(", ")} · {building.units} unidades</small><div style={{ display: "flex", gap: 7, marginTop: 8, flexWrap: "wrap" }}><span className="tag">{building.occupied} ocupadas</span><span className="tag">{Math.max(0, building.units - building.occupied)} livres</span>{isForSale(building) && <span className="tag sale-tag">À venda</span>}</div></div></Link>)}</div></div></div>;
}
