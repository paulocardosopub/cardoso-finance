...
export default function ImoveisPage() {
  ...
  const [createForm, setCreateForm] = useState({ name: "", city: "", state: "DF", value: "", units: "1", status: "active", lastValuationDate: "" });
  useEffect(() => {
    let active = true;
  ...
  async function createBuilding(event: React.FormEvent<HTMLFormElement>) {
    ...
    const buildingResult = await supabase.from("buildings").insert({ organization_id: organizationId, asset_id: asset.data.id, city: createForm.city.trim(), state: createForm.state.trim(), total_units: units, current_value: value, last_valuation_date: createForm.lastValuationDate || null, status: createForm.status }).select("id").single();
    ...
    setCreating(false);
    setCreateForm({ name: "", city: "", state: "DF", value: "", units: "1", status: "active", lastValuationDate: "" });
    await refresh();
  }
  ...
  return <div className="content"><div className="page-heading"><div><div className="eyebrow"><Building2 size={13} /> Portfólio imobiliário</div><h1>Imóveis</h1><p className="subtitle">Unidades, contratos e valores sincronizados com o Supabase.</p></div><button className="button button-primary" onClick={() => setCreating(true)} disabled={role === "viewer"}><Plus size={15} /><span>Novo imóvel</span></button></div>
    ...
    <div className="panel"><div className="panel-heading"><div><h2>{filter === "vendidos" ? "Imóveis vendidos" : "Todos os prédios e grupos"}</h2><p>{visible.length} agrupamentos · dados do banco</p></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><label className="search-inline"><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar" /></label><select className="filter-select" value={filter} onChange={(event) => setFilter(event.target.value)}><option value="todos">Ativos</option><option value="ocupados">Com ocupação</option><option value="vagos">Com vagas</option><option value="venda">À venda</option><option value="vendidos">Vendidos</option></select><SlidersHorizontal size={14} style={{ alignSelf: "center", color: "#8490a5" }} /></div></div><div className="building-list">{orderedVisible.map((building) => { const availableUnits = saleUnits(building); const forSale = isForSale(building); return <Link href={buildingPath(building)} key={building.id} className={`building-row ${forSale ? "sale-row" : ""}`} style={{ padding: 16 }}><div className="building-thumb" style={{ width: 62, height: 62 }}>{buildingPhotos[building.id] && <img src={buildingPhotos[building.id]} alt={`Foto de ${building.name}`} />}</div><div className="building-info"><strong style={{ fontSize: 14 }}>{building.name}</strong><small><MapPin size={11} style={{ verticalAlign: "-2px" }} /> {building.city}, {building.state} · {building.units} unidades</small><div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>{building.status === "vendido" && <span className="tag sale-tag">Vendido</span>}{forSale && <span className="tag sale-tag">À venda</span>}{availableUnits.length > 0 && <span className="tag sale-tag">{availableUnits.length} {availableUnits.length === 1 ? "unidade" : "unidades"}</span>}{!forSale && building.status !== "vendido" && <span className="tag">{building.status}</span>}<span className="tag">{building.occupied} ocupadas</span></div></div><div className="building-value"><strong style={{ fontSize: 15 }}>{compactBrl(building.value)}</strong><small>{brl(building.revenue)} / mês</small><div style={{ marginTop: 9, width: 100 }}><div className="progress"><span style={{ width: `${building.units ? (building.occupied / building.units) * 100 : 0}%` }} /></div></div></div></Link>; })}</div></div>
    {creating && <div className="modal-backdrop"><form className="edit-modal" onSubmit={createBuilding}><div className="panel-heading"><div><h2>Novo imóvel ou prédio</h2><p>Crie um grupo editável e suas unidades iniciais.</p></div><button type="button" className="icon-btn" onClick={() => setCreating(false)} aria-label="Fechar"><X size={16} /></button></div><div className="form-grid"><label className="form-grid-wide">Nome principal<input value={createForm.name} onChange={(event) => setCreateForm((current) => ({ ...current, name: event.target.value }))} placeholder="Ex.: Edifício Cardoso" required /></label><label>Cidade<input value={createForm.city} onChange={(event) => setCreateForm((current) => ({ ...current, city: event.target.value }))} /></label><label>Estado<input value={createForm.state} onChange={(event) => setCreateForm((current) => ({ ...current, state: event.target.value }))} maxLength={2} /></label><label>AVALIAÇÃO<input type="number" min="0" step="0.01" value={createForm.value} onChange={(event) => setCreateForm((current) => ({ ...current, value: event.target.value }))} required /></label><label>Unidades iniciais<input type="number" min="0" step="1" value={createForm.units} onChange={(event) => setCreateForm((current) => ({ ...current, units: event.target.value }))} /></label><label>Status<select value={createForm.status} onChange={(event) => setCreateForm((current) => ({ ...current, status: event.target.value }))}><option value="active">Ativo</option><option value="renovation">Reforma</option><option value="for_sale">À venda</option></select></label><label>Data da última avaliação<input type="date" value={createForm.lastValuationDate} onChange={(event) => setCreateForm((current) => ({ ...current, lastValuationDate: event.target.value }))} /></label></div><div className="onboarding-actions"><button type="button" className="button button-ghost" onClick={() => setCreating(false)}>Cancelar</button><button type="submit" className="button button-primary"><Save size={14} /> Criar imóvel</button></div></form></div>}
  </div>;
}
