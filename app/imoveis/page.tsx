...
import { Building2, CalendarDays, Check, CheckCircle2, ChevronLeft, ChevronRight, MapPin, Plus, Save, Search, SlidersHorizontal, X } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import { usePortfolio } from "@/components/portfolio-provider";
import { brl, compactBrl } from "@/lib/format";
import type { Building, LeasePaymentRecord, PropertyUnit } from "@/types/domain";
...
export default function ImoveisPage() {
  const { buildings, loading, organizationId, role, memberVisibility, memberSummary, expenses, leasePayments, refresh } = usePortfolio();
  ...
  useEffect(() => {
    ...
    if (requestedFilter === "vagos" || requestedFilter === "ocupados" || requestedFilter === "venda" || requestedFilter === "proximos" || requestedFilter === "atencao" || requestedFilter === "vendidos" || requestedFilter === "nao-pagos") setFilter(requestedFilter);
  ...
  const paymentByLease = useMemo(() => new Map(leasePayments.filter((payment) => payment.competence.startsWith(selectedMonth)).map((payment) => [payment.leaseId, payment])), [leasePayments, selectedMonth]);
  ...
  }), [buildings, filter, query, paymentByLease, selectedMonth]);
  ...
  const buildingMonthlyNet = (building: Building) => buildingMonthlyRevenue(building) - buildingMonthlyExpenses(building);
  function localToday() {
    const date = new Date();
    return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`;
  }
  function localDate(value: string) { return new Date(`${value.slice(0, 10)}T12:00:00`).toLocaleDateString("pt-BR"); }
  function requestPaymentConfirmation(building: Building, unit: PropertyUnit) {
    setPaymentMessage("");
    setPaymentConfirmation({ building, unit, values: { paymentDate: localToday(), amount: String(unit.rent), proof: null, note: "" } });
  }
  async function confirmUnpaidPayment(building: Building, unit: PropertyUnit, values: PaymentConfirmationValues) {
    if (!organizationId || !unit.rent || !isRentalMonthAvailable(selectedMonth)) return;
    const amount = Number(values.amount.replace(",", "."));
    if (!values.paymentDate || !Number.isFinite(amount) || amount <= 0) { setPaymentMessage("Informe uma data e um valor de pagamento válidos."); return; }
    const supabase = createSupabaseBrowserClient();
    if (!supabase) return;
    ...
    if (values.proof) {
      ...
      if (upload.error) { setPaymentMessage(`Não foi possível anexar o comprovante: ${upload.error}`); setPaymentSaving(false); return; }
      receiptPath = upload.path;
    }
    const result = await supabase.rpc("toggle_unit_payment", { target_org: organizationId, target_unit: unit.id, target_competence: `${selectedMonth}-01`, mark_paid: true, target_payment_date: values.paymentDate, target_amount: amount, target_receipt_path: receiptPath, target_notes: values.note.trim() || null });
    if (result.error) setPaymentMessage(result.error.message === "payment_already_exists" ? `Este aluguel já foi confirmado para ${monthLabel(selectedMonth)}.` : `Não foi possível atualizar o pagamento: ${result.error.message}`);
    else { setPaymentConfirmation(null); setPaymentMessage(`Pagamento de ${building.name} · ${unit.code} confirmado em ${localDate(values.paymentDate)}.`); await refresh(); }
  ...
  return <div className="content"><div className="page-heading"><div><div className="eyebrow"><Building2 size={13} /> Portfólio imobiliário</div><h1>Imóveis</h1><p className="subtitle">{role === "viewer" ? "Consulte os imóveis e as informações compartilhadas pela administração." : "Unidades, contratos e valores sincronizados com o Supabase."}</p></div>{role !== "viewer" && <div className="page-heading-actions"><div className="month-navigator"><button type="button" className="icon-btn" onClick={() => setSelectedMonth((month) => shiftMonth(month, -1))} aria-label="Mês anterior"><ChevronLeft size={16} /></button><CalendarDays size={15} /><strong>{monthLabel(selectedMonth)}</strong><button type="button" className="icon-btn" onClick={() => setSelectedMonth((month) => shiftMonth(month, 1))} aria-label="Próximo mês"><ChevronRight size={16} /></button></div><button className="button button-primary" onClick={() => setCreating(true)}><Plus size={15} /><span>Novo imóvel</span></button></div>}</div>
    ...
    <div className="panel"><div className="panel-heading"><div><h2>{filter === "vendidos" ? "Imóveis vendidos" : filter === "proximos" ? "Próximos a vender" : filter === "atencao" ? "Imóveis em atenção" : filter === "nao-pagos" ? "Unidades sem pagamento confirmado" : "Todos os prédios e grupos"}</h2><p>{visible.length} agrupamentos · resultado líquido de {monthLabel(currentMonth)} para a administração.</p></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><label className="search-inline"><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar" /></label>{showMemberStatus && <><select className="filter-select" value={filter} onChange={(event) => setFilter(event.target.value)}><option value="todos">Ativos</option><option value="ocupados">Com ocupação</option><option value="vagos">Com vagas</option><option value="venda">À venda</option><option value="proximos">Próximos a vender</option><option value="atencao">Atenção</option><option value="nao-pagos">Unidades sem pagamento no mês</option><option value="vendidos">Vendidos</option></select><SlidersHorizontal size={14} style={{ alignSelf: "center", color: "#8490a5" }} /></>}</div></div><div className="building-list">{filter === "nao-pagos" ? orderedVisible.map((building) => {
        ...
        return <UnpaidPropertyRow key={building.id} building={building} units={unpaidUnits} photoUrl={buildingPhotos[building.id]} paymentByLease={paymentByLease} busy={paymentSaving} onPay={requestPaymentConfirmation} />;
      }) : orderedVisible.map((building) => { const availableUnits = saleUnits(building); const forSale = isForSale(building); const monthlyExpenses = buildingMonthlyExpenses(building); const monthlyRevenue = buildingMonthlyRevenue(building); const monthlyNet = buildingMonthlyNet(building); const rowClass = `building-row ${forSale ? "sale-row" : ""} ${building.saleProximity ? "near-sale-row" : ""} ${building.attention ? "attention-row" : ""}`; return <Link href={buildingPath(building)} key={building.id} className={rowClass} style={{ padding: 16 }}><div className="building-thumb" style={{ width: 62, height: 62 }}>{buildingPhotos[building.id] && <img src={buildingPhotos[building.id]} alt={`Foto de ${building.name}`} />}</div><div className="building-info"><strong style={{ fontSize: 14 }}>{building.name}</strong><small>{showMemberLocations && <><MapPin size={11} style={{ verticalAlign: "-2px" }} /> {[building.city, building.state].filter(Boolean).join(", ")} · </>}{building.units} unidades</small>{showMemberStatus && <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>{building.status === "vendido" && <span className="tag sale-tag">Vendido</span>}{forSale && <span className="tag sale-tag">À venda</span>}{building.saleProximity && !forSale && <span className="tag near-sale-tag">Próximo a vender</span>}{building.attention && <span className="tag attention-tag">! Atenção</span>}{availableUnits.length > 0 && <span className="tag sale-tag">{availableUnits.length} {availableUnits.length === 1 ? "unidade" : "unidades"}</span>}{!forSale && building.status !== "vendido" && <span className="tag">{building.status}</span>}<span className="tag">{building.occupied} ocupadas</span></div>}</div>{(showMemberValues || showMemberRent) && <div className="building-value">{showMemberValues && <strong style={{ fontSize: 15 }}>{compactBrl(building.value)}</strong>}{showMemberRent && (role === "viewer" ? <small>{brl(monthlyRevenue)} sua parte / mês</small> : monthlyRevenue > 0 ? <><small className={monthlyNet < 0 ? "negative" : "positive"}>{brl(monthlyNet)} líquido / mês</small><small className="muted">{brl(monthlyRevenue)} aluguel · {brl(monthlyExpenses)} despesas</small></> : monthlyExpenses > 0 ? <small className="negative">{brl(monthlyNet)} líquido / mês</small> : null)}{role !== "viewer" && forSale && <button type="button" className="button button-primary button-small sale-action" onClick={(event) => { event.preventDefault(); event.stopPropagation(); setSellingBuilding(building); setSaleValue(String(building.value)); setSaleMessage(""); }}>Vender</button>}{showMemberStatus && <div style={{ marginTop: 9, width: 100 }}><div className="progress"><span style={{ width: `${building.units ? (building.occupied / building.units) * 100 : 0}%` }} /></div></div>}</div>}</Link>; })}</div></div>
    ...
    {paymentMessage && !paymentConfirmation && <p className={paymentMessage.startsWith("Não") || paymentMessage.startsWith("Este") ? "form-error" : "form-success"}><CheckCircle2 size={13} /> {paymentMessage}</p>}
    {paymentConfirmation && <PaymentConfirmationModal propertyLabel={`${paymentConfirmation.building.name} · ${paymentConfirmation.unit.code}`} month={monthLabel(selectedMonth)} values={paymentConfirmation.values} busy={paymentSaving} message={paymentMessage} onChange={(values) => setPaymentConfirmation((current) => current ? { ...current, values } : current)} onCancel={() => { setPaymentConfirmation(null); setPaymentMessage(""); }} onConfirm={(event) => { event.preventDefault(); void confirmUnpaidPayment(paymentConfirmation.building, paymentConfirmation.unit, paymentConfirmation.values); }} />}
  </div>;
}
...
function EmployeeProperties({ organizationId, buildings, buildingPhotos, query, setQuery, filter, setFilter, regionFilter, setRegionFilter }: { organizationId: string; buildings: Building[]; buildingPhotos: Record<string, string>; query: string; setQuery: (value: string) => void; filter: string; setFilter: (value: string) => void; regionFilter: string; setRegionFilter: (value: string) => void }) {
  ...
  const visible = useMemo(() => buildings.filter((building) => {
    ...
    return matches && matchesRegion && (filter === "todos" || (filter === "ocupados" && occupied) || (filter === "vagos" && !occupied) || (filter === "venda" && forSale) || (filter === "proximos" && Boolean(building.saleProximity)) || (filter === "atencao" && Boolean(building.attention)));
  ...
  return <div className="content">
    ...
    <div className="panel"><div className="panel-heading"><div><h2>Carteira operacional</h2><p>{visible.length} imóveis encontrados · {monthLabel(selectedMonth)}</p></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><label className="search-inline"><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar imóvel" /></label><select className="filter-select" value={regionFilter} onChange={(event) => setRegionFilter(event.target.value)}><option value="todas">Todas as regiões</option>{regions.map((region) => <option key={region} value={region}>{region}</option>)}</select><select className="filter-select" value={filter} onChange={(event) => setFilter(event.target.value)}><option value="todos">Todos</option><option value="ocupados">Ocupados</option><option value="vagos">Vagos</option><option value="venda">À venda</option><option value="proximos">Próximos a vender</option><option value="atencao">Atenção</option></select></div></div>
      <div className="building-list">{orderedVisible.map((building) => {
        ...
        return <Link href={buildingPath(building)} key={building.id} className={"building-row " + (isForSale(building) ? "sale-row " : "") + (building.saleProximity ? "near-sale-row " : "") + (building.attention ? "attention-row" : "")}><div className="building-thumb" style={{ width: 62, height: 62 }}>{buildingPhotos[building.id] ? <img src={buildingPhotos[building.id]} alt={"Foto de " + building.name} /> : <Building2 size={18} />}</div><div className="building-info"><strong>{building.name}</strong><small>{regionLabel(building)} · {building.units} unidades</small><div style={{ display: "flex", gap: 7, marginTop: 8, flexWrap: "wrap" }}><span className="tag">{building.occupied} ocupadas</span><span className="tag">{Math.max(0, building.units - building.occupied)} livres</span>{isForSale(building) && <span className="tag sale-tag">À venda</span>}{building.saleProximity && !isForSale(building) && <span className="tag near-sale-tag">Próximo a vender</span>}{building.attention && <span className="tag attention-tag">! Atenção</span>}</div></div><div className="building-value month-rent-summary"><strong>{brl(totals.received)} / {brl(totals.expected)}</strong><small>recebido / previsto · {progress}%</small><div className="progress"><span style={{ width: progress + "%" }} /></div></div></Link>;
      })}</div>
    </div>
  </div>;
}

function UnpaidPropertyRow({ building, units, photoUrl, paymentByLease, busy, onPay }: { building: Building; units: PropertyUnit[]; photoUrl?: string; paymentByLease: Map<string, LeasePaymentRecord>; busy: boolean; onPay: (building: Building, unit: PropertyUnit) => void }) {
  const expected = units.reduce((sum, unit) => sum + unitMonthlyRent(unit), 0);
  return <div className={`building-row unpaid-building-row ${isForSale(building) ? "sale-row" : ""} ${building.saleProximity ? "near-sale-row" : ""} ${building.attention ? "attention-row" : ""}`} style={{ padding: 16, alignItems: "flex-start" }}>
    <div className="building-thumb" style={{ width: 62, height: 62 }}>{photoUrl ? <img src={photoUrl} alt={`Foto de ${building.name}`} /> : <Building2 size={18} />}</div>
    <div className="building-info" style={{ flex: "1 1 auto", minWidth: 0 }}><strong>{building.name}</strong><small>{regionLabel(building)} · {units.length} unidade{units.length === 1 ? "" : "s"} sem confirmação</small><div style={{ display: "flex", gap: 7, marginTop: 8, flexWrap: "wrap" }}>{isForSale(building) && <span className="tag sale-tag">À venda</span>}{building.saleProximity && !isForSale(building) && <span className="tag near-sale-tag">Próximo a vender</span>}{building.attention && <span className="tag attention-tag">! Atenção</span>}<span className="tag">{units.length} pendente{units.length === 1 ? "" : "s"}</span></div><div className="employee-payment-list" style={{ marginTop: 10 }}>{units.map((unit) => { const payment = unit.lease?.id ? paymentByLease.get(unit.lease.id) : undefined; const received = Number(payment?.receivedAmount ?? 0); return <div className="payment-line" key={unit.id}><span><strong>{unit.code}</strong><small>{unit.tenantName ?? unit.tenant ?? "Inquilino não informado"}{received > 0 ? ` · parcial recebido ${brl(received)}` : " · sem confirmação"}</small></span><strong className="payment-value payment-value-pending">{brl(unit.rent)}</strong><button type="button" className="button button-primary button-small payment-button" onClick={() => onPay(building, unit)} disabled={busy}><Check size={13} /> Marcar pago</button></div>; })}</div></div>
    <div className="building-value"><strong>{brl(expected)}</strong><small>previsto no mês</small></div>
  </div>;
}
