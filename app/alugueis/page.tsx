...
import { ArrowDownRight, ArrowUpRight, CalendarClock, Check, CircleDollarSign, Edit3, Landmark, Plus, Save, Send, Trash2, X } from "lucide-react";
...
export default function AlugueisPage() {
  ...
  const currentPayments = useMemo(() => leasePayments.filter((payment) => payment.competence.startsWith(month())), [leasePayments]);
  ...
  const visiblePayments = useMemo(() => leasePayments.filter((payment) => {
  ...
  }), [leasePayments, leaseById, query, sort, statusFilter]);
  ...
  async function generateMonthCharges() {
    ...
    const rows = leases.filter((lease) => !existing.has(lease.leaseId)).map((lease) => ({ organization_id: organizationId, lease_id: lease.leaseId, competence: firstDay(), due_date: dueDateFor(month(), lease.dueDay), expected_amount: lease.rent, received_amount: 0, status: "pending" }));
    if (!rows.length) { setMessage("Todas as unidades alugadas já têm cobrança para este mês."); return; }
    setSaving(true);
    const result = await supabase.from("lease_payments").insert(rows);
    setMessage(result.error ? `Não foi possível gerar as cobranças: ${result.error.message}` : `${rows.length} cobrança(s) criada(s) para este mês.`);
    if (!result.error) await refresh();
    setSaving(false);
  }
  ...
  return <div className="content">
    <div className="page-heading"><div><div className="eyebrow"><CircleDollarSign size={13} /> Receitas imobiliárias</div><h1>Aluguéis</h1><p className="subtitle">Registre recebimentos vinculados ao imóvel e acompanhe o saldo da conta da holding.</p></div><button className="button button-primary" onClick={() => { setEditingPayment(null); setPaymentForm(blankPayment(leases[0])); }} disabled={role === "viewer"}><Plus size={14} /> Registrar aluguel</button></div>
