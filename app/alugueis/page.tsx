...
type LeaseRow = { leaseId: string; unitId: string; buildingId: string; buildingName: string; unitCode: string; tenant: string; rent: number; dueDay: number; startDate?: string | null; endDate?: string | null };
...
export default function AlugueisPage() {
  ...
  const currentPayments = useMemo(() => rentalMonthAvailable ? leasePayments.filter((payment) => payment.competence.startsWith(selectedMonth)).filter((payment) => leaseActiveInMonth(leaseById.get(payment.leaseId), selectedMonth)) : [], [leasePayments, leaseById, rentalMonthAvailable, selectedMonth]);
  const leasesForMonth = useMemo(() => rentalMonthAvailable ? leases.filter((lease) => leaseActiveInMonth(lease, selectedMonth)) : [], [leases, rentalMonthAvailable, selectedMonth]);
  ...
  const missingPayments = leasesForMonth.filter((lease) => paymentState(currentPayments.find((payment) => payment.leaseId === lease.leaseId), lease) !== "paid");
  ...
  const visiblePayments = useMemo(() => rentalMonthAvailable ? leasePayments.filter((payment) => payment.competence.startsWith(selectedMonth)).filter((payment) => {
    ...
    if (!leaseActiveInMonth(lease, selectedMonth)) return false;
  ...
  async function generateMonthCharges() {
    ...
    const rows = leasesForMonth.filter((lease) => !existing.has(lease.leaseId)).map((lease) => ({ organization_id: organizationId, lease_id: lease.leaseId, competence: firstDay(selectedMonth), due_date: dueDateFor(selectedMonth, lease.dueDay), expected_amount: lease.rent, received_amount: 0, status: "pending" }));
  ...
  async function savePayment(event: React.FormEvent<HTMLFormElement>) {
    ...
    const selectedLease = leaseById.get(paymentForm.leaseId);
    if (!selectedLease || !leaseActiveInMonth(selectedLease, paymentForm.competence.slice(0, 7))) { setMessage("A competência escolhida está fora do período do contrato."); return; }
    const supabase = createSupabaseBrowserClient(); if (!supabase) return;
    ...
    if (!result.error) { setEditingPayment(null); setPaymentForm(blankPayment(leasesForMonth[0])); await refresh(); }
    setSaving(false);
  }
  ...
  return <div className="content">
    <div className="page-heading"><div><div className="eyebrow"><CircleDollarSign size={13} /> Receitas imobiliárias</div><h1>Aluguéis</h1><p className="subtitle">Registre recebimentos vinculados ao imóvel e acompanhe o saldo da conta da holding.</p></div><div className="page-heading-actions"><div className="month-navigator"><button type="button" className="icon-btn" onClick={() => setSelectedMonth((value) => shiftMonth(value, -1))} aria-label="Mês anterior"><ChevronLeft size={16} /></button><CalendarDays size={15} /><strong>{monthLabel(selectedMonth)}</strong><button type="button" className="icon-btn" onClick={() => setSelectedMonth((value) => shiftMonth(value, 1))} aria-label="Próximo mês"><ChevronRight size={16} /></button></div><button className="button button-primary" onClick={() => { setEditingPayment(null); setPaymentForm({ ...blankPayment(leasesForMonth[0]), competence: firstDay(selectedMonth), dueDate: leasesForMonth[0] ? dueDateFor(selectedMonth, leasesForMonth[0].dueDay) : firstDay(selectedMonth) }); }} disabled={role === "viewer"}><Plus size={14} /> Registrar aluguel</button></div></div>
