...
export default function DespesasPage() {
  ...
  const expensesByPerson = useMemo(() => expenses.filter((expense) => expense.expense_kind !== "one_time" && expenseMemberId(expense)).reduce<Record<string, number>>((totals, expense) => { const id = String(expenseMemberId(expense)); totals[id] = (totals[id] ?? 0) + Number(expense.value || 0); return totals; }, {}), [expenses]);
  const holdingExpenses = expenses.filter((expense) => expense.expense_kind !== "one_time" && !expenseMemberId(expense)).reduce((total, expense) => total + Number(expense.value || 0), 0);
