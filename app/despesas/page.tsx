...
function MemberExpensesView({ expenses, buildings, loading }: { expenses: MemberExpenseRecord[]; buildings: Array<{ dbId?: string; name: string }>; loading: boolean }) {
  ...
  }).sort((left, right) => right.expense_date.localeCompare(left.expense_date)), [expenses, kindFilter, query]);
  const total = visibleExpenses.reduce((sum, expense) => sum + Number(expense.value || 0), 0);
  return <div className="content"><div className="page-heading"><div><div className="eyebrow"><CircleDollarSign size={13} /> Consulta financeira</div><h1>Despesas</h1><p className="subtitle">Despesas de sua responsabilidade e despesas gerais da holding.</p></div></div>
