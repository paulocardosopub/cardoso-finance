...
import { ArrowDownRight, CalendarDays, ChevronLeft, ChevronRight, CircleDollarSign, Download, Edit3, FileSpreadsheet, Plus, Receipt, Save, Trash2, Upload, X } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import * as XLSX from "xlsx";
import { usePortfolio } from "@/components/portfolio-provider";
import { brl } from "@/lib/format";
import { createSupabaseBrowserClient } from "@/lib/supabase";
import type { ExpenseKind, ExpenseRecord, MemberExpenseRecord } from "@/types/domain";
...
export default function DespesasPage() {
  const { buildings, loading, organizationId, role, actualRole, viewAsMemberId, expenses, monthlyExpenses, monthlyProfit, refresh } = usePortfolio();
  ...
  const [memberExpenses, setMemberExpenses] = useState<MemberExpenseRecord[]>([]);
  const [memberExpensesLoading, setMemberExpensesLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    if (!supabase) return;
    if (role === "viewer") {
      setMemberExpensesLoading(true);
      const previewingMember = actualRole !== "viewer" && viewAsMemberId;
      const previewContactId = viewAsMemberId?.startsWith("contact:") ? viewAsMemberId.slice(8) : null;
      const previewUserId = viewAsMemberId?.startsWith("user:") ? viewAsMemberId.slice(5) : (previewContactId ? null : viewAsMemberId);
      const request = previewingMember && previewContactId
        ? supabase.rpc("list_contact_member_expenses", { target_org: organizationId, target_member_contact: previewContactId })
        : previewingMember && previewUserId
          ? supabase.rpc("list_member_expenses", { target_org: organizationId, target_member_user: previewUserId })
          : supabase.rpc("list_member_expenses", { target_org: organizationId });
      request.then(({ data, error }) => {
        if (error) setMessage("Não foi possível carregar suas despesas.");
        else setMemberExpenses((data ?? []) as MemberExpenseRecord[]);
        setMemberExpensesLoading(false);
      });
      return;
    }
  ...
  const periodExpenses = useMemo(() => expenses.filter((expense) => expense.expense_kind !== "one_time" || expense.expense_date?.startsWith(selectedMonth)), [expenses, selectedMonth]);
  const periodExpenseTotal = periodExpenses.reduce((total, expense) => total + Number(expense.value || 0), 0);
  const expensesByPerson = useMemo(() => periodExpenses.reduce<Record<string, number>>((totals, expense) => { const assignments = expense.responsibilities?.length ? expense.responsibilities : expenseMemberIds(expense).map((id) => ({ user_id: id.startsWith("user:") ? id.slice(5) : undefined, contact_id: id.startsWith("contact:") ? id.slice(8) : undefined, share_percentage: 100 })); assignments.forEach((assignment) => { const id = assignment.user_id ?? assignment.contact_id ?? ""; if (id) totals[id] = (totals[id] ?? 0) + Number(expense.value || 0) * Number(assignment.share_percentage || 0) / 100; }); return totals; }, {}), [periodExpenses]);
  const holdingExpenses = periodExpenses.filter((expense) => !expense.responsibilities?.length && !expense.responsible_user_id && !expense.responsible_contact_id).reduce((total, expense) => total + Number(expense.value || 0), 0);
  const memberWeight = (member: Member) => ownershipTotal > 0 ? Number(member.ownership_percentage || 0) / ownershipTotal : members.length ? 1 / members.length : 0;
  const expenseShareForMember = (member: Member) => (expensesByPerson[member.member_id] ?? 0) + holdingExpenses * memberWeight(member);
  const visibleExpenses = useMemo(() => periodExpenses.filter((expense) => {
  ...
  async function addExpense(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (result.error) setMessage(result.error.message);
      ...
      setMessage("Despesa cadastrada e saldo atualizado."); setForm({ description: "", value: "", kind: "recurring", category: "Operacional", responsibleUserIds: [], buildingId: "", date: today() }); setCreatingExpense(false); await refresh();
    }
    setSaving(false);
  }
  ...
  if (role === "viewer") return <MemberExpensesView expenses={memberExpenses} buildings={buildings} loading={memberExpensesLoading} />;
  return <div className="content"><div className="page-heading"><div><div className="eyebrow"><CircleDollarSign size={13} /> Controle de despesas</div><h1>Despesas</h1><p className="subtitle">Despesas fixas, recorrentes e avulsas sincronizadas com o saldo financeiro. Período: {monthLabel(selectedMonth)}.</p></div><div className="month-navigator"><button type="button" className="icon-btn" onClick={() => setSelectedMonth((value) => shiftMonth(value, -1))} aria-label="Mês anterior"><ChevronLeft size={16} /></button><CalendarDays size={15} /><strong>{monthLabel(selectedMonth)}</strong><button type="button" className="icon-btn" onClick={() => setSelectedMonth((value) => shiftMonth(value, 1))} aria-label="Próximo mês"><ChevronRight size={16} /></button></div></div>
    {message && !creatingExpense && !editingExpense && <p className={message.startsWith("Erro") || message.startsWith("Não") || message.startsWith("Seu") || message.startsWith("Informe") ? "form-error" : "form-success"}><Receipt size={13} /> {message}</p>}
    <div className="metrics"><div className="metric-card"><div className="metric-top"><span>Despesas mensais</span><span className="metric-icon"><ArrowDownRight size={15} /></span></div><div className="metric-value">{brl(periodExpenseTotal)}</div><div className="metric-foot">Fixas + recorrentes + avulsas de {monthLabel(selectedMonth)}</div></div><div className="metric-card"><div className="metric-top"><span>Saldo mensal</span><span className="metric-icon">R$</span></div><div className={`metric-value ${monthlyProfit < 0 ? "negative" : ""}`}>{brl(monthlyProfit)}</div><div className="metric-foot">Receitas − despesas</div></div><div className="metric-card"><div className="metric-top"><span>Lançamentos</span><span className="metric-icon"><Receipt size={15} /></span></div><div className="metric-value">{periodExpenses.length}</div><div className="metric-foot">Lançamentos de {monthLabel(selectedMonth)}</div></div></div>
    <div className="panel expense-import-panel section-gap"><div><div className="panel-heading"><div><h2>Importar planilha</h2><p>Obrigatórias: Despesa · Data · Valor · Responsável.</p></div><FileSpreadsheet size={16} color="#80e2b0" /></div><p className="muted">O modelo também traz Mensal, Tipo, Categoria, Holding e Imóvel para automatizar o cadastro.</p></div><div className="expense-import-actions"><label className="expense-upload-button"><Upload size={14} />{importing ? "Lendo…" : "Importar Excel"}<input type="file" accept=".xlsx,.xls,.csv" onChange={(event) => void importSpreadsheet(event)} disabled={importing} /></label><button type="button" className="expense-template-link" onClick={downloadExpenseTemplate}><Download size={12} /> Baixar modelo</button></div></div>
    <div className="dashboard-grid"><div className="panel"><div className="panel-heading"><div><h2>Nova despesa</h2><p>Cadastre e atribua um ou mais responsáveis.</p></div><Plus size={17} color="#80e2b0" /></div><p className="muted">Abra o formulário para lançar uma despesa e manter a lista visível ao fundo.</p><button type="button" className="button button-primary" onClick={() => { setCreatingExpense(true); setMessage(""); }}><Plus size={15} /> Nova despesa</button></div><div className="panel"><div className="panel-heading"><div><h2>Lucro mensal por pessoa</h2><p>Receita proporcional à participação menos despesas atribuídas e a cota das despesas da holding.</p></div></div>{members.length ? <div className="table-wrap profit-table-wrap"><table className="profit-table"><thead><tr><th>Pessoa</th><th>Participação</th><th>Receita</th><th>Despesas</th><th>Lucro líquido</th></tr></thead><tbody>{members.map((member) => { const share = ownershipTotal > 0 ? (monthlyProfit + monthlyExpenses) * Number(member.ownership_percentage || 0) / ownershipTotal : (monthlyProfit + monthlyExpenses) / members.length; const assigned = expenseShareForMember(member); return <tr key={member.member_id}><td><strong>{member.full_name}</strong><small>{member.email || "Membro sem acesso"}</small></td><td>{Number(member.ownership_percentage || 0).toFixed(2).replace(".", ",")}%</td><td>{brl(share)}</td><td>{brl(assigned)}</td><td className={share - assigned < 0 ? "negative" : "positive"}><strong>{brl(share - assigned)}</strong></td></tr>; })}</tbody></table></div> : <div className="empty-state"><p>Nenhum membro disponível.</p></div>}</div></div>
    <div className="panel section-gap"><div className="panel-heading"><div><h2>Despesas cadastradas</h2><p>{visibleExpenses.length} de {expenses.length} lançamentos sincronizados.</p></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><input className="table-filter" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar descrição ou responsável" /><select className="filter-select" value={kindFilter} onChange={(event) => setKindFilter(event.target.value)}><option value="all">Todos os tipos</option><option value="fixed">Fixas mensais</option><option value="recurring">Recorrentes</option><option value="one_time">Avulsas</option></select><select className="filter-select" value={responsibleFilter} onChange={(event) => setResponsibleFilter(event.target.value)}><option value="all">Todos os responsáveis</option><option value="holding">Holding</option>{members.map((member) => <option key={member.member_id} value={memberOption(member)}>{member.full_name}</option>)}</select><select className="filter-select" value={sort} onChange={(event) => setSort(event.target.value)}><option value="date_desc">Data mais recente</option><option value="date_asc">Data mais antiga</option><option value="responsible_asc">Responsável A–Z</option><option value="kind_asc">Tipo A–Z</option><option value="value_desc">Maior valor</option><option value="value_asc">Menor valor</option></select></div></div>{visibleExpenses.length ? <div className="table-wrap"><table><thead><tr><th>Descrição</th><th>Tipo</th><th>Responsável</th><th>Imóvel</th><th>Competência</th><th>Valor</th><th>Origem</th><th>Ações</th></tr></thead><tbody>{visibleExpenses.map((expense) => <tr key={expense.id}><td><strong>{expense.description}</strong><small>{expense.category}</small></td><td><span className="tag">{kindLabels[expense.expense_kind]}</span></td><td>{expense.responsible || "Holding"}</td><td>{buildings.find((building) => building.dbId === expense.building_id)?.name ?? "Holding"}</td><td>{expense.expense_date ? new Date(`${expense.expense_date}T12:00:00`).toLocaleDateString("pt-BR") : "—"}</td><td><strong>{brl(Number(expense.value || 0))}</strong></td><td><span className="tag">{expense.created_role === "employee" ? "Funcionária" : "Administração"}</span></td><td><div className="table-actions"><button className="icon-btn" onClick={() => openEditExpense(expense)} disabled={false} aria-label={`Editar despesa ${expense.description}`}><Edit3 size={14} /></button><button className="icon-btn" onClick={() => void removeExpense(expense)} disabled={false} aria-label={`Excluir despesa ${expense.description}`}><Trash2 size={14} /></button></div></td></tr>)}</tbody></table></div> : <div className="empty-state"><Receipt size={28} /><h3>Nenhuma despesa encontrada</h3><p>Ajuste os filtros ou cadastre uma nova despesa.</p></div>}</div>
    {creatingExpense && <div className="modal-backdrop"><form className="edit-modal" onSubmit={addExpense}><div className="panel-heading"><div><h2>Nova despesa</h2><p>Cadastre e atribua um ou mais responsáveis.</p></div><button type="button" className="icon-btn" onClick={() => { setCreatingExpense(false); setMessage(""); }} aria-label="Fechar nova despesa"><X size={16} /></button></div>{message && <p className={message.startsWith("Erro") || message.startsWith("Não") || message.startsWith("Seu") || message.startsWith("Informe") ? "form-error" : "form-success"}><Receipt size={13} /> {message}</p>}<div className="form-grid"><label className="form-grid-wide">Descrição<input value={form.description} onChange={(event) => updateForm("description", event.target.value)} placeholder="Condomínio, IPTU, manutenção..." required /></label><label>Valor<input type="number" min="0" step="0.01" value={form.value} onChange={(event) => updateForm("value", event.target.value)} placeholder="0,00" required /></label><label>Tipo<select value={form.kind} onChange={(event) => updateForm("kind", event.target.value as ExpenseKind)}><option value="fixed">Fixa mensal</option><option value="recurring">Recorrente</option><option value="one_time">Avulsa</option></select></label><label>Categoria<input value={form.category} onChange={(event) => updateForm("category", event.target.value)} /></label><label>Competência<input type="date" value={form.date} onChange={(event) => updateForm("date", event.target.value)} required /></label><label>Responsáveis <small className="muted">Selecione vários com Ctrl/Cmd</small><select multiple size={4} value={form.responsibleUserIds} onChange={(event) => setForm((current) => ({ ...current, responsibleUserIds: Array.from(event.target.selectedOptions, (option) => option.value) }))}>{members.map((member) => <option key={member.member_id} value={memberOption(member)}>{member.full_name} · {member.ownership_percentage || 0}%{member.is_placeholder ? " · sem acesso" : ""}</option>)}</select></label><label>Imóvel relacionado<select value={form.buildingId} onChange={(event) => updateForm("buildingId", event.target.value)}><option value="">Holding / todos</option>{buildings.filter((building) => building.dbId).map((building) => <option key={building.dbId} value={building.dbId}>{building.name}</option>)}</select></label><div className="form-grid-wide onboarding-actions"><span className="muted">Sem responsáveis, a despesa é rateada entre todos conforme a participação.</span><button type="submit" className="button button-primary" disabled={saving}>{saving ? "Salvando…" : "Cadastrar despesa"}</button></div></div><div className="onboarding-actions"><button type="button" className="button button-ghost" onClick={() => setCreatingExpense(false)}>Cancelar</button></div></form></div>}
    {editingExpense && <div className="modal-backdrop"><form className="edit-modal" onSubmit={saveEditedExpense}><div className="panel-heading"><div><h2>Editar despesa</h2><p>Atualize o lançamento e recalcule o saldo automaticamente.</p></div><button type="button" className="icon-btn" onClick={() => { setEditingExpense(null); setMessage(""); }} aria-label="Fechar edição"><X size={16} /></button></div>{message && <p className={message.startsWith("Erro") || message.startsWith("Não") || message.startsWith("Seu") || message.startsWith("Informe") ? "form-error" : "form-success"}><Receipt size={13} /> {message}</p>}<div className="form-grid"><label className="form-grid-wide">Descrição<input value={form.description} onChange={(event) => updateForm("description", event.target.value)} required /></label><label>Valor<input type="number" min="0" step="0.01" value={form.value} onChange={(event) => updateForm("value", event.target.value)} required /></label><label>Tipo<select value={form.kind} onChange={(event) => updateForm("kind", event.target.value)}><option value="fixed">Fixa mensal</option><option value="recurring">Recorrente</option><option value="one_time">Avulsa</option></select></label><label>Categoria<input value={form.category} onChange={(event) => updateForm("category", event.target.value)} /></label><label>Competência<input type="date" value={form.date} onChange={(event) => updateForm("date", event.target.value)} required /></label><label>Responsáveis <small className="muted">Selecione vários com Ctrl/Cmd</small><select multiple size={4} value={form.responsibleUserIds} onChange={(event) => setForm((current) => ({ ...current, responsibleUserIds: Array.from(event.target.selectedOptions, (option) => option.value) }))}>{members.map((member) => <option key={member.member_id} value={memberOption(member)}>{member.full_name}{member.is_placeholder ? " · sem acesso" : ""}</option>)}</select></label><label className="form-grid-wide">Imóvel relacionado<select value={form.buildingId} onChange={(event) => updateForm("buildingId", event.target.value)}><option value="">Holding / todos</option>{buildings.filter((building) => building.dbId).map((building) => <option key={building.dbId} value={building.dbId}>{building.name}</option>)}</select></label></div><div className="onboarding-actions"><button type="button" className="button button-ghost" onClick={() => setEditingExpense(null)}>Cancelar</button><button type="submit" className="button button-primary" disabled={saving}><Save size={14} /> {saving ? "Salvando…" : "Salvar alterações"}</button></div></form></div>}
  </div>;
}

function MemberExpensesView({ expenses, buildings, loading }: { expenses: MemberExpenseRecord[]; buildings: Array<{ dbId?: string; name: string }>; loading: boolean }) {
  const [query, setQuery] = useState("");
  const [kindFilter, setKindFilter] = useState("all");
  const [selectedMonth, setSelectedMonth] = useState(currentMonthKey);
  const periodExpenses = useMemo(() => expenses.filter((expense) => expense.expense_kind !== "one_time" || expense.expense_date?.startsWith(selectedMonth)), [expenses, selectedMonth]);
  const visibleExpenses = useMemo(() => periodExpenses.filter((expense) => {
    const text = `${expense.description} ${expense.category} ${expense.responsible}`.toLowerCase();
    return text.includes(query.toLowerCase()) && (kindFilter === "all" || expense.expense_kind === kindFilter);
  }).sort((left, right) => right.expense_date.localeCompare(left.expense_date)), [periodExpenses, kindFilter, query]);
  const total = visibleExpenses.reduce((sum, expense) => sum + Number(expense.value || 0), 0);
  return <div className="content"><div className="page-heading"><div><div className="eyebrow"><CircleDollarSign size={13} /> Consulta financeira</div><h1>Despesas</h1><p className="subtitle">Despesas de sua responsabilidade e despesas gerais da holding. Período: {monthLabel(selectedMonth)}.</p></div><div className="month-navigator"><button type="button" className="icon-btn" onClick={() => setSelectedMonth((value) => shiftMonth(value, -1))} aria-label="Mês anterior"><ChevronLeft size={16} /></button><CalendarDays size={15} /><strong>{monthLabel(selectedMonth)}</strong><button type="button" className="icon-btn" onClick={() => setSelectedMonth((value) => shiftMonth(value, 1))} aria-label="Próximo mês"><ChevronRight size={16} /></button></div></div>
    <div className="metrics"><div className="metric-card"><div className="metric-top"><span>Despesas visíveis</span><span className="metric-icon"><Receipt size={15} /></span></div><div className="metric-value">{visibleExpenses.length}</div><div className="metric-foot">Lançamentos autorizados</div></div><div className="metric-card"><div className="metric-top"><span>Valor exibido</span><span className="metric-icon">R$</span></div><div className="metric-value">{brl(total)}</div><div className="metric-foot">Sua responsabilidade + holding</div></div></div>
    <div className="panel section-gap"><div className="panel-heading"><div><h2>Despesas autorizadas</h2><p>Despesas de outros membros não aparecem nesta tela.</p></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><input className="table-filter" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar descrição" /><select className="filter-select" value={kindFilter} onChange={(event) => setKindFilter(event.target.value)}><option value="all">Todos os tipos</option><option value="fixed">Fixas mensais</option><option value="recurring">Recorrentes</option><option value="one_time">Avulsas</option></select></div></div>{loading ? <div className="empty-state"><p>Carregando despesas…</p></div> : visibleExpenses.length ? <div className="table-wrap"><table><thead><tr><th>Descrição</th><th>Tipo</th><th>Responsabilidade</th><th>Imóvel</th><th>Competência</th><th>Valor</th></tr></thead><tbody>{visibleExpenses.map((expense) => <tr key={expense.id}><td><strong>{expense.description}</strong><small>{expense.category}</small></td><td><span className="tag">{kindLabels[expense.expense_kind]}</span></td><td><span className="tag">{expense.is_holding_expense ? "Holding" : "Sua responsabilidade"}</span></td><td>{buildings.find((building) => building.dbId === expense.building_id)?.name ?? "Holding"}</td><td>{expense.expense_date ? new Date(`${expense.expense_date}T12:00:00`).toLocaleDateString("pt-BR") : "—"}</td><td><strong>{brl(Number(expense.value || 0))}</strong></td></tr>)}</tbody></table></div> : <div className="empty-state"><Receipt size={28} /><h3>Nenhuma despesa encontrada</h3><p>Você só verá despesas gerais ou atribuídas à sua responsabilidade.</p></div>}</div>
  </div>;
}
