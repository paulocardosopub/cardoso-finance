...
export default function DespesasPage() {
  ...
  const expenseMemberId = (expense: ExpenseRecord) => expense.responsible_user_id ?? expense.responsible_contact_id ?? null;
  ...
  const expensesByPerson = useMemo(() => expenses.filter((expense) => isIncludedInCurrentMonth(expense) && expenseMemberId(expense)).reduce<Record<string, number>>((totals, expense) => { const id = String(expenseMemberId(expense)); totals[id] = (totals[id] ?? 0) + Number(expense.value || 0); return totals; }, {}), [expenses]);
  const holdingExpenses = expenses.filter((expense) => isIncludedInCurrentMonth(expense) && !expenseMemberId(expense)).reduce((total, expense) => total + Number(expense.value || 0), 0);
  const visibleExpenses = useMemo(() => expenses.filter((expense) => {
    ...
    return text.includes(query.toLowerCase()) && (kindFilter === "all" || expense.expense_kind === kindFilter) && (responsibleFilter === "all" || (expenseMemberId(expense) ?? "holding") === responsibleFilter);
  ...
  async function addExpense(event: React.FormEvent<HTMLFormElement>) {
    ...
    const supabase = createSupabaseBrowserClient();
    if (!supabase) return;
    const member = members.find((item) => memberOption(item) === form.responsibleUserId);
    setSaving(true);
    const result = await supabase.from("expenses").insert({ organization_id: organizationId, description: form.description.trim(), category: form.category.trim() || "Operacional", value, expense_date: form.date || today(), competence: form.date || today(), recurring: form.kind !== "one_time", expense_kind: form.kind, responsible: member?.full_name ?? "Holding", responsible_user_id: member?.user_id || null, responsible_contact_id: member?.contact_id || null, building_id: form.buildingId || null });
  ...
  function openEditExpense(expense: ExpenseRecord) {
    const responsibleUserId = expense.responsible_user_id ? `user:${expense.responsible_user_id}` : expense.responsible_contact_id ? `contact:${expense.responsible_contact_id}` : "";
    setForm({ description: expense.description, value: String(expense.value), kind: expense.expense_kind, category: expense.category || "Operacional", responsibleUserId, buildingId: expense.building_id ?? "", date: expense.expense_date || today() });
  ...
  async function saveEditedExpense(event: React.FormEvent<HTMLFormElement>) {
    ...
    const member = members.find((item) => memberOption(item) === form.responsibleUserId);
    setSaving(true);
    const result = await supabase.from("expenses").update({ description: form.description.trim(), category: form.category.trim() || "Operacional", value, expense_date: form.date || today(), competence: form.date || today(), recurring: form.kind !== "one_time", expense_kind: form.kind, responsible: member?.full_name ?? "Holding", responsible_user_id: member?.user_id || null, responsible_contact_id: member?.contact_id || null, building_id: form.buildingId || null }).eq("id", editingExpense.id).eq("organization_id", organizationId);
    if (result.error) setMessage(`Não foi possível atualizar: ${result.error.message}`);
    else { setMessage("Despesa atualizada e saldo recalculado."); setEditingExpense(null); await refresh(); }
    setSaving(false);
  }
