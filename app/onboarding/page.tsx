...
export default function OnboardingPage() {
  ...
  async function createOrganization() {
    ...
    setLoading(true);
    const isPaulo = (await supabase.auth.getUser()).data.user?.email?.toLowerCase() === "paulocardosopub@gmail.com";
    const { data: organizationId, error: organizationError } = await supabase.rpc("create_organization", { org_name: isPaulo ? "Cardoso" : choice === "personal" ? "Meu patrimônio" : "Minha organização", org_type: isPaulo || choice === "holding" ? "company" : "personal", org_description: isPaulo ? "Holding Cardoso · dados importados da planilha enviada" : "Organização vazia criada pelo usuário", org_currency: "BRL" });
    if (organizationError) { setError(organizationError.message); setLoading(false); return; }
    try { if (isPaulo) await syncInitialPortfolio(organizationId); router.push("/"); } catch (syncError) { setError(syncError instanceof Error ? syncError.message : "Não foi possível importar os imóveis."); setLoading(false); }
  }
