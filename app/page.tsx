...
export default function DashboardPage() {
  const { buildings, notifications, leasePayments, loading, organizationId, userName, bankBalance, monthlyExpenses, monthlyProfit, role, actualRole, viewAsMemberId, previewMembers, memberVisibility, memberSummary, ownershipSummary, refresh } = usePortfolio();
  ...
  if (role === "viewer") return <MemberDashboard buildings={activeBuildings} organizationId={organizationId} userName={userName} viewedMemberName={actualRole !== "viewer" ? previewMembers.find((member) => member.userId === viewAsMemberId)?.name : undefined} visibility={memberVisibility} summary={memberSummary} ownership={ownershipSummary} />;
...
function MemberDashboard({ buildings, organizationId, userName, viewedMemberName, visibility, summary, ownership }: { buildings: Building[]; organizationId: string; userName: string; viewedMemberName?: string; visibility: ReturnType<typeof usePortfolio>["memberVisibility"]; summary: ReturnType<typeof usePortfolio>["memberSummary"]; ownership: ReturnType<typeof usePortfolio>["ownershipSummary"] }) {
  ...
  return <div className="content member-dashboard">
    ...
    <section className="metrics member-dashboard-metrics">
      {visibility.showTotalAssets && <><Metric icon={<CircleDollarSign size={15} />} label="Seu patrimônio" value={compactBrl(summary.totalValue)} foot="Sua participação proporcional" positive /><Metric icon={<Landmark size={15} />} label="Patrimônio total da holding" value={compactBrl(summary.holdingTotalValue)} foot="Valor total dos imóveis" positive /></>}
      ...
      {visibility.showRentalInfo && <Metric icon={<ArrowUpRight size={15} />} label="Sua receita líquida mensal" value={brl(summary.totalRent)} foot={`${summary.ownershipPercentage.toFixed(2).replace(".", ",")}% da receita, após despesas`} positive={summary.totalRent >= 0} />}
    </section>
    <section className="dashboard-grid">
