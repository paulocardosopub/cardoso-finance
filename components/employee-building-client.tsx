...
export function EmployeeBuildingClient({ building }: { building: Building }) {
  ...
  async function togglePayment(unit: PropertyUnit, markPaid: boolean) {
    ...
    setMessage(result.error ? `Não foi possível atualizar o pagamento${result.error.message ? `: ${result.error.message}` : "."}` : (markPaid ? "Pagamento confirmado e crédito criado." : "Pagamento desmarcado e crédito desfeito."));
