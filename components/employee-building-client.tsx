...
export function EmployeeBuildingClient({ building }: { building: Building }) {
  ...
  const [paymentConfirmation, setPaymentConfirmation] = useState<{ unit: PropertyUnit; values: PaymentConfirmationValues } | null>(null);
  ...
  function requestPaymentConfirmation(unit: PropertyUnit) {
    setPaymentConfirmation({ unit, values: { paymentDate: localToday(), amount: String(unit.rent), proof: null, note: "" } });
  }
  async function togglePayment(unit: PropertyUnit, markPaid: boolean, values?: PaymentConfirmationValues) {
    ...
    const amount = markPaid && values ? Number(values.amount.replace(",", ".")) : null;
    const paymentDate = markPaid && values ? values.paymentDate : null;
    if (markPaid && (!paymentDate || !Number.isFinite(amount) || Number(amount) <= 0)) { setMessage("Informe uma data e um valor de pagamento válidos."); setBusy(false); return; }
    let receiptPath: string | null = null;
    if (markPaid && values?.proof) {
      const upload = await uploadPaymentProof(supabase, organizationId, building.dbId ?? building.id, unit.id, selectedMonth, values.proof);
    ...
    setMessage(result.error ? `Não foi possível atualizar o pagamento${result.error.message ? `: ${result.error.message}` : "."}` : (markPaid ? `Pagamento de ${monthLabel(selectedMonth)} confirmado em ${paymentDate ? localDate(paymentDate) : now.toLocaleDateString("pt-BR")}. Crédito criado.` : `Pagamento de ${monthLabel(selectedMonth)} desmarcado em ${now.toLocaleDateString("pt-BR")}. Crédito desfeito.`));
