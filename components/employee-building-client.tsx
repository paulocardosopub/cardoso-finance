...
export function EmployeeBuildingClient({ building }: { building: Building }) {
  ...
  async function togglePayment(unit: PropertyUnit, markPaid: boolean, values?: PaymentConfirmationValues) {
    ...
    if (!result.error) { const paymentId = String(result.data?.paymentId ?? unit.lease?.id ?? ""); setPayments((rows) => markPaid ? [...rows.filter((row) => row.lease_id !== unit.lease?.id), { lease_id: unit.lease?.id ?? paymentId, status: "paid", received_amount: amount ?? unit.rent, expected_amount: unit.rent, received_at: paymentDate ?? now.toISOString() }] : rows.filter((row) => row.lease_id !== unit.lease?.id)); setPaymentConfirmation(null); await refresh(); }
    setBusy(false);
  }
