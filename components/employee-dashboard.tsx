...
import { Building2, Check, CheckCircle2, Clock3, MapPin, Tag, UserRound, X } from "lucide-react";
...
export function EmployeeDashboard({ buildings, organizationId, userName, refresh }: { buildings: Building[]; organizationId: string; userName: string; refresh: () => Promise<void> }) {
  ...
  const [recentVisits, setRecentVisits] = useState<Array<{ id: string; building_id: string; unit_id?: string | null; visited_at: string; latitude?: number | null; longitude?: number | null }>>([]);
  useEffect(() => {
    const supabase = createSupabaseBrowserClient(); if (!supabase) return;
    supabase.from("property_visits").select("id, building_id, unit_id, visited_at, latitude, longitude").eq("organization_id", organizationId).order("visited_at", { ascending: false }).limit(6).then(({ data }) => setRecentVisits((data ?? []) as typeof recentVisits));
  }, [organizationId]);
  ...
  const openPayments = rented.filter(({ unit }) => unit.lease?.currentPaymentStatus !== "paid");
  async function togglePayment(unit: PropertyUnit, paid: boolean) {
    if (!unit.lease?.id) return;
    const supabase = createSupabaseBrowserClient(); if (!supabase) return;
    setBusy(`payment:${unit.id}`); setMessage("");
    const result = await supabase.rpc("toggle_lease_payment", { target_org: organizationId, target_lease: unit.lease.id, target_competence: `${new Date().toISOString().slice(0, 7)}-01`, mark_paid: paid });
    setMessage(result.error ? (result.error.message === "payment_already_exists" ? "Este aluguel já foi confirmado neste mês." : "Não foi possível atualizar o pagamento.") : (paid ? "Pagamento confirmado e crédito criado." : "Pagamento desmarcado e crédito desfeito."));
  ...
  return <div className="content"><div className="page-heading"><div><div className="eyebrow"><Building2 size={13} /> Painel operacional</div><h1>Olá, {userName}!</h1><p className="subtitle">Acompanhe ocupação, pagamentos e visitas dos imóveis sob sua responsabilidade.</p></div><span className="tag">Funcionária</span></div>
    ...
    <section className="dashboard-grid"><div className="panel"><div className="panel-heading"><div><h2>Pagamentos do mês</h2><p>Confirme cada aluguel uma única vez por competência.</p></div><Check size={17} color="#80e2b0" /></div>{rented.length ? <div className="employee-payment-list">{rented.map(({ building, unit }) => { const paid = unit.lease?.currentPaymentStatus === "paid"; return <div className="payment-line" key={unit.id}><span><strong>{building.name} · {unit.code}</strong><small>{unit.tenantName ?? unit.lease?.tenantName ?? "Inquilino não informado"}</small></span><button type="button" className={paid ? "button button-ghost button-small" : "button button-primary button-small"} disabled={busy === `payment:${unit.id}`} onClick={() => void togglePayment(unit, !paid)}>{paid ? <><Check size={13} /> Pago · desfazer</> : <><X size={13} /> Marcar pago</>}</button></div>; })}</div> : <div className="empty-state"><p>Nenhum contrato ativo encontrado.</p></div>}</div><div className="panel"><div className="panel-heading"><div><h2>Visitas aos imóveis</h2><p>Registre presença com data, hora e localização do aparelho.</p></div><MapPin size={17} color="#80e2b0" /></div>{active.length ? <div className="employee-visit-list">{active.map((building) => <div className="payment-line" key={building.id}><span><strong>{building.name}</strong><small>{building.city}{building.state ? ` · ${building.state}` : ""}</small></span><button type="button" className="button button-ghost button-small" disabled={busy === `visit:${building.id}`} onClick={() => void markVisited(building)}><MapPin size={13} /> {busy === `visit:${building.id}` ? "Registrando…" : "Visitado"}</button></div>)}</div> : <div className="empty-state"><p>Nenhum imóvel cadastrado.</p></div>}{recentVisits.length > 0 && <div className="activity-list" style={{ marginTop: 16 }}>{recentVisits.map((visit) => { const building = buildings.find((item) => item.dbId === visit.building_id); const unit = building?.unitsData?.find((item) => item.id === visit.unit_id); return <div className="activity-item" key={visit.id}><strong>{building?.name ?? "Imóvel"}{unit ? ` · ${unit.code}` : ""}</strong><small>{new Date(visit.visited_at).toLocaleString("pt-BR")} {visit.latitude != null && visit.longitude != null ? "· localização registrada" : "· sem localização"}</small></div>; })}</div>}</div></section>
