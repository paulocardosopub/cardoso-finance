-- Allow operational payment confirmations to record an adjusted amount and reason.
-- Keep legacy RPC signatures working for existing administrator screens.
...
declare
  ...
  property_label text;
  payment_note text := nullif(trim(coalesce(target_notes, '')), '');
begin
  ...
  select p.* into payment_row
   ...
   where p.lease_id = target_lease
     and p.competence = competence_date
   for update;
  previous_received := coalesce(payment_row.received_amount, 0);

  if payment_row.id is null then
    insert into public.lease_payments(
      lease_id, organization_id, competence, due_date, expected_amount,
      received_amount, received_at, receipt_path, status, notes
    ) values (
      target_lease, target_org, competence_date,
      competence_date + greatest(0, least(30, lease_row.due_day - 1)),
      expected, received,
      case when mark_paid then payment_date else null end,
      target_receipt_path,
      (case when mark_paid then 'paid' else 'pending' end)::public.payment_status,
      coalesce(payment_note, 'Confirmação operacional')
    ...
    update public.lease_payments
       set expected_amount = expected,
           ...
           notes = coalesce(payment_note, notes, 'Confirmação operacional'),
  ...
  property_label := 'Aluguel · ' || coalesce(asset_name, 'Imóvel')
    || coalesce(' · ' || nullif(unit_code, ''), '')
    || coalesce(' · ' || nullif(tenant_name, ''), '');

  if mark_paid then
    insert into public.revenues(
      organization_id, asset_id, value, revenue_date, competence, category,
      description, origin, notes, created_by, source_payment_id
    ) values (
      target_org, asset_id, received, payment_date, competence_date, 'rent',
      property_label, 'lease_payment',
      coalesce(payment_note, 'Criado pela confirmação do pagamento'),
      auth.uid(), payment_row.id
    )
    ...
    if exists (select 1 from public.financial_history h where h.source_payment_id = payment_row.id and h.event_type = 'credit') then
      update public.financial_history
       ...
       where source_payment_id = payment_row.id and event_type = 'credit';
    else
  ...
  return jsonb_build_object(
    ...
    'paymentDate', payment_row.received_at,
    'receiptPath', payment_row.receipt_path,
    'notes', payment_row.notes,
    'markedPaid', mark_paid,
    'description', property_label
  );
end;
$$;
...
returns jsonb language sql security definer set search_path = public as $$
  select public.toggle_lease_payment(
    target_org, target_lease, target_competence, mark_paid,
    target_payment_date, target_amount, target_receipt_path, null
  );
$$;
...
returns jsonb language sql security definer set search_path = public as $$
  select public.toggle_lease_payment(target_org, target_lease, target_competence, mark_paid, null, null, null, null);
$$;
...
begin
  ...
  if lease_row.id is null then
    ...
    if unit_rent <= 0 or unit_row.status = 'sold'::public.unit_status then raise exception 'lease_not_found'; end if;
    if not mark_paid then raise exception 'lease_not_found'; end if;
  ...
  return public.toggle_lease_payment(
    ...
    target_payment_date, target_amount, target_receipt_path, target_notes
  );
end;
$$;
...
returns jsonb language sql security definer set search_path = public as $$
  select public.toggle_unit_payment(
    target_org, target_unit, target_competence, mark_paid,
    target_payment_date, target_amount, target_receipt_path, null
  );
$$;
...
returns jsonb language sql security definer set search_path = public as $$
  select public.toggle_unit_payment(target_org, target_unit, target_competence, mark_paid, null, null, null, null);
$$;

revoke all on function public.toggle_lease_payment(uuid, uuid, date, boolean, date, numeric, text, text) from public, anon;
grant execute on function public.toggle_lease_payment(uuid, uuid, date, boolean, date, numeric, text, text) to authenticated;
...
revoke all on function public.toggle_unit_payment(uuid, uuid, date, boolean, date, numeric, text, text) from public, anon;
grant execute on function public.toggle_unit_payment(uuid, uuid, date, boolean, date, numeric, text, text) to authenticated;
