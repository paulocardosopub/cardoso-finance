...
begin
  ...
  select m.user_id, m.user_id, null::uuid,
    ...
    u.email::text, m.role, m.ownership_percentage, m.joined_at, false
  from public.organization_members m join auth.users u on u.id = m.user_id left join public.profiles p on p.id = m.user_id
  where m.organization_id = target_org
  union all
  select c.id, null::uuid, c.id, c.full_name::text, c.email::text, c.role, c.ownership_percentage, c.created_at, true
  from public.organization_contacts c where c.organization_id = target_org
