...
begin
  ...
  if viewer_role is null then raise exception 'not_authorized'; end if;
  if viewer_role not in ('viewer', 'owner', 'admin', 'manager') then raise exception 'member_endpoint_only'; end if;
  select coalesce(sum(ownership_percentage), 0) into ownership_total from (select ownership_percentage from public.organization_members where organization_id = target_org union all select ownership_percentage from public.organization_contacts where organization_id = target_org) owners;
  member_weight := case when ownership_total > 0 then viewer_ownership / ownership_total else 0 end;
  select jsonb_build_object(
    'month', month_start,
    'ownershipPercentage', viewer_ownership,
    'rentalIncome', coalesce((select sum(round(coalesce((select l.current_rent from public.leases l where l.organization_id = target_org and l.unit_id = u.id and l.status in ('active','ending') and l.start_date <= month_end and (l.end_date is null or l.end_date >= month_start) order by l.updated_at desc limit 1), u.potential_rent, 0) * member_weight, 2)) from public.property_units u join public.buildings b on b.id = u.building_id where u.organization_id = target_org and b.status <> 'sold'), 0),
    'individualCredits', coalesce((select sum(r.value) from public.revenues r where r.organization_id = target_org and r.beneficiary_user_id = auth.uid() and ((coalesce(r.recurring, false) and r.revenue_date <= month_end) or (not coalesce(r.recurring, false) and coalesce(r.competence, r.revenue_date) >= month_start and coalesce(r.competence, r.revenue_date) < next_month))), 0),
    'ownExpenses', coalesce((select sum(case when exists (select 1 from public.expense_responsibilities er where er.organization_id = target_org and er.expense_id = e.id) then coalesce((select sum(er.share_percentage) from public.expense_responsibilities er where er.organization_id = target_org and er.expense_id = e.id and er.user_id = auth.uid()), 0) * e.value / 100 when e.responsible_user_id = auth.uid() then e.value else 0 end) from public.expenses e where e.organization_id = target_org and (coalesce(e.expense_kind, 'recurring') <> 'one_time' or (e.expense_date >= month_start and e.expense_date < next_month)) and (exists (select 1 from public.expense_responsibilities er where er.organization_id = target_org and er.expense_id = e.id and er.user_id = auth.uid()) or (not exists (select 1 from public.expense_responsibilities er where er.organization_id = target_org and er.expense_id = e.id) and e.responsible_user_id = auth.uid()))), 0),
    'holdingExpenses', coalesce((select sum(e.value * member_weight) from public.expenses e where e.organization_id = target_org and (coalesce(e.expense_kind, 'recurring') <> 'one_time' or (e.expense_date >= month_start and e.expense_date < next_month)) and not exists (select 1 from public.expense_responsibilities er where er.organization_id = target_org and er.expense_id = e.id) and e.responsible_user_id is null and e.responsible_contact_id is null), 0),
    'credits', coalesce((select jsonb_agg(jsonb_build_object('id', r.id, 'value', r.value, 'revenueDate', r.revenue_date, 'competence', coalesce(r.competence, r.revenue_date), 'description', r.description, 'origin', r.origin, 'recurring', coalesce(r.recurring, false)) order by coalesce(r.competence, r.revenue_date) desc, r.created_at desc) from public.revenues r where r.organization_id = target_org and r.beneficiary_user_id = auth.uid() and ((coalesce(r.recurring, false) and r.revenue_date <= month_end) or (not coalesce(r.recurring, false) and coalesce(r.competence, r.revenue_date) >= month_start and coalesce(r.competence, r.revenue_date) < next_month))), '[]'::jsonb),
    'expenses', coalesce((select jsonb_agg(jsonb_build_object('id', e.id, 'description', e.description, 'category', e.category, 'value', case when exists (select 1 from public.expense_responsibilities er where er.organization_id = target_org and er.expense_id = e.id) then coalesce((select sum(er.share_percentage) from public.expense_responsibilities er where er.organization_id = target_org and er.expense_id = e.id and er.user_id = auth.uid()), 0) * e.value / 100 when e.responsible_user_id = auth.uid() then e.value else e.value * member_weight end, 'expenseDate', e.expense_date, 'expenseKind', coalesce(e.expense_kind, 'recurring'), 'scope', case when exists (select 1 from public.expense_responsibilities er where er.organization_id = target_org and er.expense_id = e.id) or e.responsible_user_id = auth.uid() then 'individual' else 'holding' end) order by e.expense_date desc, e.created_at desc) from public.expenses e where e.organization_id = target_org and (coalesce(e.expense_kind, 'recurring') <> 'one_time' or (e.expense_date >= month_start and e.expense_date < next_month)) and (exists (select 1 from public.expense_responsibilities er where er.organization_id = target_org and er.expense_id = e.id and er.user_id = auth.uid()) or (not exists (select 1 from public.expense_responsibilities er where er.organization_id = target_org and er.expense_id = e.id) and (e.responsible_user_id = auth.uid() or (e.responsible_user_id is null and e.responsible_contact_id is null)))), '[]'::jsonb)
  ) into result;
  result := result || jsonb_build_object('netTotal', coalesce((result->>'rentalIncome')::numeric, 0) + coalesce((result->>'individualCredits')::numeric, 0) - coalesce((result->>'ownExpenses')::numeric, 0) - coalesce((result->>'holdingExpenses')::numeric, 0));
  return result;
end;
$$;
