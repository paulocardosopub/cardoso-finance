-- Importação protegida: a carteira da planilha só pode ser replicada pelo
-- usuário Paulo Cardoso, a partir da holding Cardoso já auditada.
...
begin
  ...
  select id into source_org from public.organizations
    where owner_id = auth.uid() and name = 'Cardoso' order by created_at limit 1;
