-- Mantém o aluguel esperado histórico após encerramento do contrato atual.
-- A data do contrato limita a competência; o status histórico da unidade decide se ela estava alugada no mês.
-- Assim, uma unidade alugada a partir de setembro não aparece como pendente em agosto.
  ...
  from (
    with month_series as (
      ...
      select ms.month_start,
        ...
        coalesce(sum(coalesce(u.quantity,1)) filter (where u.created_at < (ms.month_start + interval '1 month') and (coalesce(status_state.value,u.fallback_status) = 'for_sale' or (coalesce(status_state.value,u.fallback_status) = 'rented' and lease_state.active is not null))),0)::integer as occupied_units,
        coalesce(sum(case when u.created_at < (ms.month_start + interval '1 month') and not (coalesce(status_state.value,u.fallback_status) = 'for_sale' or (coalesce(status_state.value,u.fallback_status) = 'rented' and lease_state.active is not null)) then coalesce(u.quantity,1) else 0 end),0)::integer as vacant_units,
        case when ms.month_start < date '2026-08-01' then 0::numeric else coalesce(sum(case when u.created_at < (ms.month_start + interval '1 month') and (coalesce(status_state.value,u.fallback_status) = 'for_sale' or (coalesce(status_state.value,u.fallback_status) = 'rented' and lease_state.active is not null) or (coalesce(status_state.value,u.fallback_status) = 'for_sale') ) then coalesce(rent_state.value,u.fallback_rent) * coalesce(u.quantity,1) else 0 end),0)::numeric(18,2) end as monthly_income
      ...
      left join lateral (
        ...
        where l.organization_id=target_org and l.unit_id=u.id
          and l.start_date < (ms.month_start + interval '1 month')::date
